...
# ── Storage location ──────────────────────────────────────────────────────────
# On AWS: /opt/trading/data/  (writable by the trading service user)
# On Mac: ~/.options_briefing/ (original local path, kept as fallback)
_DATA_DIR_ENV = os.environ.get('TRADING_DATA_DIR', '')
if _DATA_DIR_ENV:
    PORTFOLIO_DIR = Path(_DATA_DIR_ENV)
else:
    PORTFOLIO_DIR = Path.home() / '.options_briefing'
DB_PATH           = PORTFOLIO_DIR / 'portfolio.db'
STARTING_BALANCE  = 10_000.0   # Default paper trading starting balance
...
class Portfolio:
    ...
    def update_current_price(self, trade_id: int, price: float):
        """Alias for update_price — used by the position monitor."""
        self.update_price(trade_id, price)
    ...
    def get_open_positions(self) -> list[dict]:
        ...
        with self._conn() as conn:
            ...
            for r in rows:
                d = dict(r)
                ep = d['entry_price']
                raw_cp = d.get('current_price') or ep

                # Sanity check: if stored current_price looks like a stock price
                # (> 20× the option entry price), the old bug wrote a stock price here.
                # Fall back to entry_price until the position monitor corrects it.
                cp = raw_cp if (ep <= 0 or raw_cp <= ep * 25) else ep
    ...
    def get_summary(self) -> dict:
        ...
        # ── Account balance ────────────────────────────────────────────
        starting      = float(self.get_setting('starting_balance', str(STARTING_BALANCE)))
        acct_balance  = starting + realized_pnl          # cash balance grows/shrinks with closed trades
        invested      = total_open_cost                  # money currently tied up in open positions
        available     = acct_balance - invested          # spendable right now
        ...
        return {
            ...
            'account_balance':  round(acct_balance, 2),
            'available_cash':   round(available, 2),
            'invested':         round(invested, 2),
        }

    def get_account_balance(self) -> dict:
        """Convenience method — returns just account-level numbers."""
        s = self.get_summary()
        return {
    ...
    def set_starting_balance(self, amount: float):
        """Set the starting paper-trading balance."""
        self.set_setting('starting_balance', str(float(amount)))
