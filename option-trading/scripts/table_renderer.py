...
def _detail_panel(c):
    ...
    strike   = c['strike']
    expiry   = c['expiry']
    chart_color = '#3fb950' if is_call else '#f85149'

    chart_section = f"""
    <div style="background:{SURFACE2};border:1px solid {BORDER};border-radius:8px;
                padding:16px;margin-bottom:20px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
        <span style="color:{TEXT};font-weight:700;font-size:14px;">
          📈 {ticker} — 60-Day Price Chart
        </span>
        <span id="chart-status-{ticker}" style="color:{TEXT2};font-size:11px;">Loading live data...</span>
      </div>
      <canvas id="chart-{ticker}" height="200"
              style="width:100%;max-height:200px;display:block;"></canvas>
      <div style="display:flex;gap:16px;margin-top:10px;flex-wrap:wrap;">
        <span style="color:{TEXT2};font-size:11px;">
          Strike: <span style="color:{chart_color};font-weight:700;">${strike:.0f}</span>
          {opt_type} — Expiry: <span style="color:{TEXT};">{expiry}</span>
        </span>
        <span style="color:{TEXT2};font-size:11px;" id="chart-price-{ticker}">
          Current: <span style="color:{TEAL};font-weight:700;">--</span>
        </span>
        <span style="color:{TEXT2};font-size:11px;" id="chart-chg-{ticker}">
          Change: <span style="font-weight:700;">--</span>
        </span>
      </div>
    </div>
    <script>
    (function() {{
      if (window._chartLoaded__{ticker}) return;
      window._chartLoaded__{ticker} = true;
      function doLoadChart() {{
        if (typeof Chart === 'undefined') {{
          var s = document.createElement('script');
          s.src = 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.min.js';
          s.onload = function() {{ renderChart_{ticker}(); }};
          document.head.appendChild(s);
        }} else {{
          renderChart_{ticker}();
        }}
      }}
      function renderChart_{ticker}() {{
        var statusEl = document.getElementById('chart-status-{ticker}');
        fetch('/api/chart/{ticker}')
          .then(function(r) {{ return r.json(); }})
          .then(function(d) {{
            if (d.error) {{ if(statusEl) statusEl.textContent = 'No data available'; return; }}
            var canvas = document.getElementById('chart-{ticker}');
            if (!canvas) return;
            var lastClose = d.closes[d.closes.length - 1];
            var prevClose = d.closes[d.closes.length - 2] || lastClose;
            var chg = ((lastClose - prevClose) / prevClose * 100).toFixed(2);
            var chgColor = chg >= 0 ? '{GREEN}' : '{RED}';
            if(statusEl) statusEl.textContent = 'Live (15-min delay)';
            var priceEl = document.getElementById('chart-price-{ticker}');
            if(priceEl) priceEl.innerHTML = 'Current: <span style="color:{TEAL};font-weight:700;">$' + lastClose.toFixed(2) + '</span>';
            var chgEl = document.getElementById('chart-chg-{ticker}');
            if(chgEl) chgEl.innerHTML = 'Change: <span style="color:' + chgColor + ';font-weight:700;">' + (chg >= 0 ? '+' : '') + chg + '%</span>';
            // Strike line data
            var strikeArr = d.dates.map(function() {{ return {strike}; }});
            new Chart(canvas, {{
              type: 'line',
              data: {{
                labels: d.dates,
          ...
          .catch(function() {{
            if(statusEl) statusEl.textContent = 'Chart unavailable offline';
          }});
      }}
      doLoadChart();
    }})();
...
def _row_with_detail(rank, c, uid):
    ...
    <tr class="pick-row" data-uid="{uid}" data-score="{c['score']}" data-ticker="{c['ticker']}"
      ...
      <td style="padding:11px 8px;text-align:right;" id="sp-{c['ticker']}">
        <span style="color:{TEAL};font-weight:700;font-size:13px;"
              data-live-ticker="{c['ticker']}" data-base-price="{c['price']:.2f}">
          ${c['price']:.2f}
        </span>
        <div style="font-size:10px;" id="sp-chg-{c['ticker']}" data-change-ticker="{c['ticker']}"></div>
      </td>
      <td style="padding:11px 8px;text-align:right;">
        <span style="color:{YELLOW};font-weight:700;font-size:13px;"
              id="op-{uid}" data-entry="{c['entry']:.2f}">
          ${c['entry']:.2f}
        </span>
        <div style="color:{TEXT2};font-size:10px;">calc'd</div>
      </td>
    ...
    <tr id="detail-{uid}" style="display:none;">
      <td colspan="19" style="padding:0 8px 12px;background:{BG};">
        {_detail_panel(c)}
      </td>
    </tr>"""
...
def _table_section(tf_key, tf_info, candidates):
    ...
    <tr style="background:{SURFACE2};border-bottom:2px solid {header_color};">
      ...
      <th style="padding:10px 8px;color:{TEAL};font-size:11px;cursor:pointer;" title="Live stock price (updates every 30s)">STOCK $<br><span style="font-size:9px;opacity:0.6;">LIVE</span></th>
      <th style="padding:10px 8px;color:{YELLOW};font-size:11px;" title="Option entry price (Black-Scholes)">OPT $<br><span style="font-size:9px;opacity:0.6;">BID/MID</span></th>
  ...
  fetch('/api/prices')
    ...
    .then(function(prices) {{
      Object.keys(prices).forEach(function(ticker) {{
        var p = prices[ticker];
        var priceEls = document.querySelectorAll('[data-live-ticker="' + ticker + '"]');
        priceEls.forEach(function(el) {{
          el.textContent = '$' + p.price.toFixed(2);
          el.style.color = p.change >= 0 ? '{GREEN}' : '{RED}';
        }});
        var chgEls = document.querySelectorAll('[data-change-ticker="' + ticker + '"]');
        chgEls.forEach(function(el) {{
          var sign = p.change >= 0 ? '+' : '';
          el.textContent = sign + p.change.toFixed(2) + '%';
          el.style.color = p.change >= 0 ? '{GREEN}' : '{RED}';
        }});
      }});
      var ts = new Date().toLocaleTimeString();
      var el = document.getElementById('price-update-ts');
      if (el) el.textContent = 'Prices updated ' + ts;
    }})
    .catch(function() {{ /* silently ignore if offline */ }});
}}

// Poll immediately + every 30s
pollLivePrices();
...
def render_live_dashboard(ranked, date_str, vix, regime_counts,
    ...
    """
    # If scan hasn't completed yet, show loading screen that auto-retries
    total_picks = sum(len(v) for v in ranked.values()) if ranked else 0
    if total_picks == 0:
        return f"""<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Options Dashboard — Loading...</title>
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .spinner {{ width:56px; height:56px; border:5px solid #30363d;
              border-top-color:#58a6ff; border-radius:50%;
              animation:spin 0.9s linear infinite; margin:0 auto 24px; }}
  @keyframes spin {{ to {{ transform:rotate(360deg); }} }}
  h2 {{ font-size:22px; color:#f0f6fc; margin-bottom:10px; }}
...
<div>
  ...
  <h2>📈 Scanning {len(ranked) and 79 or 79} tickers<span class="dot">.</span><span class="dot">.</span><span class="dot">.</span></h2>
  <p>Building your options dashboard — this takes about 15 seconds on first load.</p>
  <p style="margin-top:8px;color:#58a6ff;">Page will refresh automatically when ready.</p>
</div>
